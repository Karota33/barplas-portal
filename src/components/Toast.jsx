...
export function ToastProvider({ children }) {
  ...
  const error = (message, duration = 6000) => addToast(message, 'error', duration) // Errores duran más
...
// useToast hook moved to src/utils/toastUtils.js to avoid fast-refresh violations
// Import from there: import { useToast } from '../utils/toastUtils'
...
function ToastItem({ toast, onRemove }) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (remaining <= 300) { // Iniciar animación de salida 300ms antes
        setIsExiting(true)
      }
    }, 50)
  ...
  return (
    <div
      className={`
        relative max-w-sm w-full border rounded-lg shadow-lg pointer-events-auto
